...
void CModel::InitializeBasins(const optStruct &Options,const bool re_init)
{
  ...
  /*ofstream TMP;
  string filename=FilenamePrepare("RoutingInformation.txt",Options);
  TMP.open(filename);
  for (int p = 0; p < _nSubBasins; p++) {
    TMP<<"basin "<<_pSubBasins[p]->GetID()<<endl;
    TMP<<"  mannings:    "<<_pSubBasins[p]->GetBasinProperties("MANNINGS_N")<<endl;
    TMP<<"  diffusivity: "<<_pSubBasins[p]->GetBasinProperties("DIFFUSIVITY")<<endl;
    TMP<<"  celerity:    "<<_pSubBasins[p]->GetBasinProperties("CELERITY")<<endl;
    TMP<<"  Q_ref:       "<<_pSubBasins[p]->GetBasinProperties("Q_REFERENCE")<<endl;
    const double*aR=_pSubBasins[p]->GetRoutingHydrograph();
