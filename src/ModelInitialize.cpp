...
void CModel::InitializeBasins(const optStruct &Options,const bool re_init)
{
  ...
  { //calculated in order from upstream to downstream
    ...
    if (_pSubBasins[p]->GetReachLength()   == AUTO_COMPUTE){warn3=true;}
    if (_pSubBasins[p]->GetTimeOfConc()    == AUTO_COMPUTE){warn4=true;}
    if(!re_init) {
  ...
  if ((warn3) && (_nSubBasins>2)){
    WriteAdvisory("CModel::InitializeBasins: one or more subbasin reach lengths were autogenerated from subbasin area", Options.noisy);
  }
  if ((warn4) && (_nSubBasins>1)){
    WriteAdvisory("CModel::InitializeBasins: one or more subbasin times of concentration and/or times to peak were autogenerated from subbasin area", Options.noisy);
  }
...
void CModel::InitializeParameterOverrides()
{
  for (int i=0;i<_nParamOverrides;i++)
  {
    ...
    for (int k=0;k<_nHydroUnits;k++){
      long long SBID=_pSubBasins[_pHydroUnits[k]->GetSubBasinIndex()]->GetID();
