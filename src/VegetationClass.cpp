...
void CVegetationClass::SummarizeToScreen()
{
  ...
  for (int c=0; c<NumVegClasses;c++){
    cout<<"-Veg. class \""<<pAllVegClasses[c]->GetTag()<<"\" "<<endl;
...
//////////////////////////////////////////////////////////////////
/// \brief Write vegetation class properties to file
/// \param &OUT [out] Output stream to which information is written
//
void CVegetationClass::WriteParamsToFile(ofstream &OUT)
{
  CVegetationClass *s;
  const veg_struct *t;
  OUT<<endl<<"---Vegetation Parameters---------------------"<<endl;
  OUT<<"CLASS,";
  OUT<<"MAX_HEIGHT[m],MAX_LEAF_COND[mm/s],MAX_LAI[m2/m2],";
  OUT<<"SVF_EXTINCTION[-],RAIN_ICEPT_FACT[-],SNOW_ICEPT_FACT[-],RAIN_ICEPT_PCT[-],SNOW_ICEPT_PCT[-],";
  OUT<<"SAI_HT_RATIO[m2/m3],TRUNK_FRACTION[0..1],STEMFLOW_FRAC[0..1],";
  OUT<<"ALBEDO[-],ALBEDO_WET[-],";
  OUT<<"MAX_CAPACITY[mm],MAX_SNOW_CAPACITY[mm],";
  OUT<<"DRIP_PROPORTION[1/d],MAX_INTERCEPT_RATE[mm/d],CHU_MATURITY[-]";
  OUT<<"ROOT_EXTINCT[-],";
  OUT<<endl;
  for (int c=0; c<NumVegClasses;c++)
  {
    s=pAllVegClasses[c];
    t=s->GetVegetationStruct();
    OUT<<s->GetTag()<<",";
    OUT<<t->max_height<<","<<t->max_leaf_cond<<","<<t->max_LAI<<",";
    OUT<<t->svf_extinction<<","<<t->rain_icept_fact<<","<<t->snow_icept_fact<<","<<t->rain_icept_pct<<","<<t->snow_icept_pct<<",";
    OUT<<t->SAI_ht_ratio<<","<<t->trunk_fraction<<","<<t->stemflow_frac<<",";
    OUT<<t->albedo<<","<<t->albedo_wet<<",";
    OUT<<t->max_capacity<<","<<t->max_snow_capacity<<",";
    OUT<<t->drip_proportion<<","<<t->max_intercept_rate <<","<<t->CHU_maturity <<",";
    OUT<<t->root_extinct<<",";
...
CVegetationClass *CVegetationClass::StringToVegClass(const string s)
{
  ...
  for (int c=0;c<NumVegClasses;c++)
  {
    if (!sup.compare(StringToUppercase(pAllVegClasses[c]->GetTag()))){return pAllVegClasses[c];}
...
//
string vegautolist[]={"SVF_EXTINCTION","ALBEDO","ALBEDO_WET","RAIN_ICEPT_FACT",
                      "SNOW_ICEPT_FACT"};
void CVegetationClass::AutoCalculateVegetationProps(const veg_struct &Vtmp, const veg_struct &Vdefault)
{
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter SVF_EXTINCTION for vegetation class "+tag+" was autogenerated with value "+to_string(V.svf_extinction);
  ...
  if (autocalc)
  {
    //RELATIONSHIP REQUIRED
    V.albedo=0.14;
    warn="The required parameter ALBEDO for vegetation class "+tag+" was autogenerated with value "+to_string(V.albedo);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter ALBEDO_WET for vegetation class "+tag+" was autogenerated with value "+to_string(V.albedo_wet);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter RAIN_ICEPT_FACT for vegetation class "+tag+" was autogenerated with value "+to_string(V.rain_icept_fact);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter SNOW_ICEPT_FACT for vegetation class "+tag+" was autogenerated with value "+to_string(V.snow_icept_fact);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter RAIN_ICEPT_PCT for vegetation class "+tag+" was autogenerated with value "+to_string(V.rain_icept_pct);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter SNOW_ICEPT_PCT for vegetation class "+tag+" was autogenerated with value "+to_string(V.snow_icept_pct);
    if(chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter MAX_CAPACITY for vegetation class "+tag+" was autogenerated with value "+to_string(V.max_capacity);
    if (chatty){WriteWarning(warn,false);}
  }
  ...
  if (autocalc)
  {
    ...
    warn="The required parameter MAX_SNOW_CAPACITY for vegetation class "+tag+" was autogenerated with value "+to_string(V.max_snow_capacity);
    if (chatty){WriteWarning(warn,false);}
  }
  ...
  if (is_auto){
    warn="The required array parameter RELATIVE_LAI[] for vegetation class "+tag+" was autogenerated";
...
//
void CVegetationClass::InitializeVegetationProps(veg_struct &V, bool is_template)
{
  //Required parameters
  V.max_height          =25;
