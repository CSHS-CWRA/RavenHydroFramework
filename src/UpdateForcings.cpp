                                       ...
                                       const time_struct &tt)
{
  ...
  double tmp_precip            ;
  double tmp_precip_daily_ave  ;
  double tmp_precip_5day       ;
  double tmp_snow_frac         ;
  double tmp_temp_ave          ;
  double tmp_temp_daily_ave    ;
  double tmp_temp_daily_min    ;
  double tmp_temp_daily_max    ;
  double tmp_temp_month_min    ;
  double tmp_temp_month_max    ;
  double tmp_temp_month_ave    ;

  // double time_idx_chunk_temp;                       // idx of current simulated time point in current gridded data chunk
  // double time_idx_chunk_day_temp;                   // idx of first time point of current day
  // double time_idx_chunk_temp_ave;                   // idx of current simulated time point in current gridded data chunk
  // double time_idx_chunk_day_temp_ave;               // idx of first time point of current day
  // double time_idx_chunk_pre;                        // idx of current simulated time point in current gridded data chunk
  // double time_idx_chunk_day_pre;                    // idx of first time point of current day 
  ...
  double interval_tave;                             // delta t of various gridded forcing data
  ...
  pre_gridded            = ForcingGridIsInput("PRECIP")         ; // ForcingGridIsAvailable("PRECIP")         ; // 
  rain_gridded           = ForcingGridIsInput("RAINFALL")       ; // ForcingGridIsAvailable("RAINFALL")       ; // 
  snow_gridded           = ForcingGridIsInput("SNOWFALL")       ; // ForcingGridIsAvailable("SNOWFALL")       ; // 
  pet_gridded            = ForcingGridIsInput("PET")            ; // ForcingGridIsAvailable("PET")            ; // 
  temp_ave_gridded       = ForcingGridIsInput("TEMP_AVE")       ; // ForcingGridIsAvailable("TEMP_AVE")       ; // 
  temp_daily_min_gridded = ForcingGridIsInput("TEMP_DAILY_MIN") ; // ForcingGridIsAvailable("TEMP_DAILY_MIN") ; // 
  temp_daily_max_gridded = ForcingGridIsInput("TEMP_DAILY_MAX") ; // ForcingGridIsAvailable("TEMP_DAILY_MAX") ; // 
  temp_daily_ave_gridded = ForcingGridIsInput("TEMP_DAILY_AVE") ; // ForcingGridIsAvailable("TEMP_DAILY_AVE") ; //

  // printf("pre_gridded            : input = %i   derived = %i \n",pre_gridded,            ForcingGridIsAvailable("PRECIP"        ));
  // printf("rain_gridded           : input = %i   derived = %i \n",rain_gridded,           ForcingGridIsAvailable("RAINFALL"      ));
  // printf("snow_gridded           : input = %i   derived = %i \n",snow_gridded,           ForcingGridIsAvailable("SNOWFALL"      ));
  // printf("pet_gridded            : input = %i   derived = %i \n",pet_gridded,            ForcingGridIsAvailable("PET"           ));
  // printf("temp_ave_gridded       : input = %i   derived = %i \n",temp_ave_gridded,       ForcingGridIsAvailable("TEMP_AVE"      ));
  // printf("temp_daily_min_gridded : input = %i   derived = %i \n",temp_daily_min_gridded, ForcingGridIsAvailable("TEMP_DAILY_MIN"));
  // printf("temp_daily_max_gridded : input = %i   derived = %i \n",temp_daily_max_gridded, ForcingGridIsAvailable("TEMP_DAILY_MAX"));
  // printf("temp_daily_ave_gridded : input = %i   derived = %i \n",temp_daily_ave_gridded, ForcingGridIsAvailable("TEMP_DAILY_AVE"));
  ...
  for (g=0;g<_nGauges;g++)
    {
      ...
      // Fg[g].precip          =_pGauges[g]->GetForcingValue    (F_PRECIP        ,nn);    
      ...
      if ( !(pre_gridded || snow_gridded || rain_gridded) )      { Fg[g].precip          =_pGauges[g]->GetForcingValue    (F_PRECIP        ,nn);    } //mm/d  
      if ( !(pre_gridded || snow_gridded || rain_gridded) )      { Fg[g].precip_daily_ave=_pGauges[g]->GetForcingValue    (F_PRECIP, model_day, 1); }
      if ( !(pre_gridded || snow_gridded || rain_gridded) )      { Fg[g].precip_5day     =_pGauges[g]->GetForcingValue    (F_PRECIP,t-5.0,5.0)*5.0; }
      if ( !(pre_gridded || rain_gridded || snow_gridded) )      { Fg[g].snow_frac       =_pGauges[g]->GetAverageSnowFrac (nn); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_ave        =_pGauges[g]->GetForcingValue    (F_TEMP_AVE      ,nn); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_daily_ave  =_pGauges[g]->GetForcingValue    (F_TEMP_DAILY_AVE,nn); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_daily_min  =_pGauges[g]->GetForcingValue    (F_TEMP_DAILY_MIN,nn); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_daily_max  =_pGauges[g]->GetForcingValue    (F_TEMP_DAILY_MAX,nn); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_month_min  =_pGauges[g]->GetMonthlyMinTemp  (mo); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_month_max  =_pGauges[g]->GetMonthlyMaxTemp  (mo); }
      if ( !(temp_daily_min_gridded && temp_daily_max_gridded) ) { Fg[g].temp_month_ave  =_pGauges[g]->GetMonthlyAveTemp  (mo); }
            
      // Fg[g].temp_ave_unc    =Fg[g].temp_daily_ave;   // will be overwritten at the end of routine
      // Fg[g].temp_min_unc    =Fg[g].temp_daily_min;   // will be overwritten at the end of routine
      // Fg[g].temp_max_unc    =Fg[g].temp_daily_max;   // will be overwritten at the end of routine

      // if ( !pet_gridded ) { Fg[g].PET_month_ave   =_pGauges[g]->GetMonthlyAvePET   (mo); }
  ...
  for (k = 0; k < _nHydroUnits; k++)
    {
      ...
      for (g = 0; g < _nGauges; g++)
        {
          ...
          if (wt != 0.0){
            ...
            if ( !(pre_gridded || snow_gridded || rain_gridded) )                                   		       { F.precip           += wt * Fg[g].precip;            }
            if ( !(pre_gridded || snow_gridded || rain_gridded) )                                   		       { F.precip_daily_ave += wt * Fg[g].precip_daily_ave;  }    
            if ( !(pre_gridded || snow_gridded || rain_gridded) )                                   		       { F.precip_5day      += wt * Fg[g].precip_5day;       }
            if ( !(pre_gridded || snow_gridded || rain_gridded) )                                                      { F.snow_frac        += wt * Fg[g].snow_frac;         }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_ave         += wt * Fg[g].temp_ave;          }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_daily_ave   += wt * Fg[g].temp_daily_ave;    }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_daily_min   += wt * Fg[g].temp_daily_min;    }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_daily_max   += wt * Fg[g].temp_daily_max;    }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_month_min   += wt * Fg[g].temp_month_min;    }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_month_max   += wt * Fg[g].temp_month_max;    }
            if ( !(temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded) ) { F.temp_month_ave   += wt * Fg[g].temp_month_ave;    }  
      ...
      // printf("(1) F.precip            %f\n",F.precip          );
      // printf("(1) F.precip_daily_ave  %f\n",F.precip_daily_ave);
      // printf("(1) F.precip_5day       %f\n",F.precip_5day     );
      // printf("(1) F.snow_frac         %f\n",F.snow_frac       );
      // printf("(1) F.temp_ave          %f\n",F.temp_ave        );
      // printf("(1) F.temp_daily_ave    %f\n",F.temp_daily_ave  );
      // printf("(1) F.temp_daily_min    %f\n",F.temp_daily_min  );
      // printf("(1) F.temp_daily_max    %f\n",F.temp_daily_max  );
      // printf("(1) F.temp_month_min    %f\n",F.temp_month_min  );
      // printf("(1) F.temp_month_max    %f\n",F.temp_month_max  );
      // printf("(1) F.temp_month_ave    %f\n",F.temp_month_ave  );

      // tmp_precip            = F.precip          ;        
      // tmp_precip_daily_ave  = F.precip_daily_ave;
      // tmp_precip_5day       = F.precip_5day     ;
      // tmp_snow_frac         = F.snow_frac       ;
      // tmp_temp_ave          = F.temp_ave        ;
      // tmp_temp_daily_ave    = F.temp_daily_ave  ;
      // tmp_temp_daily_min    = F.temp_daily_min  ;
      // tmp_temp_daily_max    = F.temp_daily_max  ;
      // tmp_temp_month_min    = F.temp_month_min  ;
      // tmp_temp_month_max    = F.temp_month_max  ;
      // tmp_temp_month_ave    = F.temp_month_ave  ;
      ...
      if ( pre_gridded)             { pGrid_pre        = GetForcingGrid(GetForcingGridIndexFromName("PRECIP")        ); } // printf("PRECIP\n");}
      if ( rain_gridded)            { pGrid_rain       = GetForcingGrid(GetForcingGridIndexFromName("RAINFALL")      ); } // printf("RAINFALL\n");}
      if ( snow_gridded)            { pGrid_snow       = GetForcingGrid(GetForcingGridIndexFromName("SNOWFALL")      ); } // printf("SNOWFALL\n");}
      if ( pet_gridded)             { pGrid_pet        = GetForcingGrid(GetForcingGridIndexFromName("PET")           ); } // printf("PET\n");}
      if ( temp_ave_gridded )       { pGrid_tave       = GetForcingGrid(GetForcingGridIndexFromName("TEMP_AVE")      ); } // printf("TEMP_AVE\n");}     
      if ( temp_daily_min_gridded ) { pGrid_daily_tmin = GetForcingGrid(GetForcingGridIndexFromName("TEMP_DAILY_MIN")); } // printf("TEMP_DAILY_MIN\n");}
      if ( temp_daily_max_gridded ) { pGrid_daily_tmax = GetForcingGrid(GetForcingGridIndexFromName("TEMP_DAILY_MAX")); } // printf("TEMP_DAILY_MAX\n");}
      if ( temp_daily_ave_gridded ) { pGrid_daily_tave = GetForcingGrid(GetForcingGridIndexFromName("TEMP_DAILY_AVE")); } // printf("TEMP_DAILY_AVE\n");}
      ...
      if ( pre_gridded || snow_gridded || rain_gridded )      {
        ...
        if ( pre_gridded || (snow_gridded && rain_gridded) )    { F.precip           = 0.0; }
        if ( pre_gridded || (snow_gridded && rain_gridded) )    { F.precip_daily_ave = 0.0; }
        if ( pre_gridded || (snow_gridded && rain_gridded) )    { F.precip_5day      = 0.0; }
        if ( pre_gridded || (snow_gridded && rain_gridded) )    { F.snow_frac        = 0.0; }
	...
	nNonZeroGridCells      = pGrid_snow->GetNumberNonZeroGridCells();
      ...
      if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded ) {
        ...
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_ave         = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_daily_ave   = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_daily_min   = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_daily_max   = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_month_min   = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_month_max   = 0.0; }
        if ( temp_ave_gridded || (temp_daily_min_gridded && temp_daily_max_gridded) || temp_daily_ave_gridded )    { F.temp_month_ave   = 0.0; }
	...
	nNonZeroGridCells = pGrid_tave->GetNumberNonZeroGridCells();
        ...
        // WriteWarning("CModel::UpdateHRUForcingFunctions: temp_month_min is not calculated when gridded input data are used",BAD_DATA);
        // WriteWarning("CModel::UpdateHRUForcingFunctions: temp_month_max is not calculated when gridded input data are used",BAD_DATA);
        // WriteWarning("CModel::UpdateHRUForcingFunctions: temp_month_ave is not calculated when gridded input data are used",BAD_DATA);

      }

      // printf("(2) F.precip            %f\n",F.precip          );
      // printf("(2) F.precip_daily_ave  %f\n",F.precip_daily_ave);
      // printf("(2) F.precip_5day       %f\n",F.precip_5day     );
      // printf("(2) F.snow_frac         %f\n",F.snow_frac       );
      // printf("(2) F.temp_ave          %f\n",F.temp_ave        );
      // printf("(2) F.temp_daily_ave    %f\n",F.temp_daily_ave  );
      // printf("(2) F.temp_daily_min    %f\n",F.temp_daily_min  );
      // printf("(2) F.temp_daily_max    %f\n",F.temp_daily_max  );
      // printf("(2) F.temp_month_min    %f\n",F.temp_month_min  );
      // printf("(2) F.temp_month_max    %f\n",F.temp_month_max  );
      // printf("(2) F.temp_month_ave    %f\n",F.temp_month_ave  );

      // bool diff = false;
      
      // if ( abs(tmp_precip            - F.precip          ) > 0.00001 )
      //        { printf("t = %f    F.precip           = %18.9f F.precip           = %18.9f <<<\n",t,tmp_precip             , F.precip          ); diff = true; }
      // // else
      // //     { printf("t = %f    F.precip           = %f F.precip           = %f    \n",t,tmp_precip             , F.precip          ); }       
      // if ( abs(tmp_precip_daily_ave  - F.precip_daily_ave) > 0.00001 )
      //        { printf("t = %f    F.precip_daily_ave = %f F.precip_daily_ave = %f <<<\n",t,tmp_precip_daily_ave   , F.precip_daily_ave); diff = true; }
      // // if ( abs(tmp_precip_5day       - F.precip_5day     ) > 0.000001 )
      // //     { printf("t = %f    F.precip_5day      = %f F.precip_5day      = %f <<<\n",t,tmp_precip_5day        , F.precip_5day     ); diff = true; }       
      // if ( abs(tmp_snow_frac         - F.snow_frac       ) > 0.00001 )
      //        { printf("t = %f    F.snow_frac        = %f F.snow_frac        = %f <<<\n",t,tmp_snow_frac          , F.snow_frac       ); diff = true; }   
      // if ( abs(tmp_temp_ave          - F.temp_ave        ) > 0.00001 )
      //        { printf("t = %f    F.temp_ave         = %f F.temp_ave         = %f <<< \n",t,tmp_temp_ave           , F.temp_ave        ); diff = true; }  
      // // else
      // //     { printf("t = %f    F.temp_ave         = %f F.temp_ave         = %f \n",t,tmp_temp_ave           , F.temp_ave        ); }       
      // if ( abs(tmp_temp_daily_ave    - F.temp_daily_ave  ) > 0.00001 )
      //        { printf("t = %f    F.temp_daily_ave   = %f F.temp_daily_ave   = %f <<<\n",t,tmp_temp_daily_ave     , F.temp_daily_ave  ); diff = true; }  
      // if ( abs(tmp_temp_daily_min    - F.temp_daily_min  ) > 0.00001 )
      //        { printf("t = %f    F.temp_daily_min   = %f F.temp_daily_min   = %f <<<\n",t,tmp_temp_daily_min     , F.temp_daily_min  ); diff = true; }  
      // if ( abs(tmp_temp_daily_max    - F.temp_daily_max  ) > 0.00001 )
      //        { printf("t = %f    F.temp_daily_max   = %f F.temp_daily_max   = %f <<<\n",t,tmp_temp_daily_max     , F.temp_daily_max  ); diff = true; }  
      // if ( abs(tmp_temp_month_min    - F.temp_month_min  ) > 0.00001 )
      //        { printf("t = %f    F.temp_month_min   = %f F.temp_month_min   = %f <<<\n",t,tmp_temp_month_min     , F.temp_month_min  ); diff = true; }  
      // if ( abs(tmp_temp_month_max    - F.temp_month_max  ) > 0.00001 )
      //        { printf("t = %f    F.temp_month_max   = %f F.temp_month_max   = %f <<<\n",t,tmp_temp_month_max     , F.temp_month_max  ); diff = true; }  
      // if ( abs(tmp_temp_month_ave    - F.temp_month_ave  ) > 0.00001 )
      //        { printf("t = %f    F.temp_month_ave   = %f F.temp_month_ave   = %f <<<\n",t,tmp_temp_month_ave     , F.temp_month_ave  ); diff = true; }  
      // if (diff) {exit(0);}
      ...
      //--Gauge or ForcingGrid Corrections------------------------------------------------
      double gauge_corr;
      double grid_corr;
      ...
      // printf("(3) F.precip            %f\n",F.precip          );
      // printf("(3) F.precip_daily_ave  %f\n",F.precip_daily_ave);
      // printf("(3) F.precip_5day       %f\n",F.precip_5day     );
      // printf("(3) F.snow_frac         %f\n",F.snow_frac       );
      // printf("(3) F.temp_ave          %f\n",F.temp_ave        );
      // printf("(3) F.temp_daily_ave    %f\n",F.temp_daily_ave  );
      // printf("(3) F.temp_daily_min    %f\n",F.temp_daily_min  );
      // printf("(3) F.temp_daily_max    %f\n",F.temp_daily_max  );
      // printf("(3) F.temp_month_min    %f\n",F.temp_month_min  );
      // printf("(3) F.temp_month_max    %f\n",F.temp_month_max  );
      // printf("(3) F.temp_month_ave    %f\n",F.temp_month_ave  );
...
void CModel::InitializeForcingGrids(const optStruct &Options, const string type)
{
  ...
  int interval;   // delta t of a specific forcing grid
